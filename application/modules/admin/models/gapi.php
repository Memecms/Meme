...
class Admin_Model_gapi
{
  ...
  {
    $response = $this->httpRequest(Admin_Model_gapi::account_data_url, array('start-index'=>$start_index,'max-results'=>$max_results), null, $this->generateAuthHeader());
  ...
  public function requestReportData($report_id, $dimensions, $metrics, $sort_metric=null, $filter=null, $start_date=null, $end_date=null, $start_index=1, $max_results=30)
  {
    ...
    $parameters['prettyprint'] = Admin_Model_gapi::dev_mode ? 'true' : 'false';
    
    $response = $this->httpRequest(Admin_Model_gapi::report_data_url, $parameters, null, $this->generateAuthHeader());
  ...
  protected function authenticateUser($email, $password)
  {
    $post_variables = array(
      ...
      'Email' => $email,
      'Passwd' => $password,
      'source' => Admin_Model_gapi::interface_name,
      'service' => 'analytics'
    );
    
    $response = $this->httpRequest(Admin_Model_gapi::client_login_url,null,$post_variables);
  ...
  {
    $interface = Admin_Model_gapi::http_interface;
    
    if(Admin_Model_gapi::http_interface =='auto')
    {
    ...
    {
      throw new Exception('Invalid http interface defined. No such interface "' . Admin_Model_gapi::http_interface . '"');
    }
  }
  ...
  public function __call($name,$parameters)
  {
    ...
    $parameter_key = Admin_Model_gapi::array_key_exists_nc($name,$this->report_root_parameters);
    ...
    $aggregate_metric_key = Admin_Model_gapi::array_key_exists_nc($name,$this->report_aggregate_metrics);
...
class gapiAccountEntry
{
  ...
  public function __call($name,$parameters)
  {
    ...
    $property_key = Admin_Model_gapi::array_key_exists_nc($name,$this->properties);
...
class gapiReportEntry
{
  ...
  public function __call($name,$parameters)
  {
    ...
    $metric_key = Admin_Model_gapi::array_key_exists_nc($name,$this->metrics);
    ...
    $dimension_key = Admin_Model_gapi::array_key_exists_nc($name,$this->dimensions);
