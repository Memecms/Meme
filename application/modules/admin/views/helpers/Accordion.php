 ...
 * @version		$Id: Accordion.php 401 2010-11-18 20:25:30Z alex $
 */


class Zend_View_Helper_Accordion extends Zend_View_Helper_Abstract {    
    
        public function Accordion( $who, $active = false) {
        
        $jquery = $this->view->jQuery()->enable()->uiEnable();
		
		$jqHandler = ZendX_JQuery_View_Helper_JQuery::getJQueryHandler();
 
 		if($active == true){$active = ', active:false';}else{$active = '';}
 
        $function = '("'.$who.'").accordion({collapsible:true'.$active.'});';
