...
class Zend_View_Helper_Products_Category {

 
    function Products_Category() {
    
        $meme = new Admin_Model_MemeStart();
    	$CategoryList = $meme->getProductCategory();
		
		$categories = '<div id="category"><h3>Products Categories</h3>';
		if(count($CategoryList)):
			foreach($CategoryList as $category):
				
				if($category['category_type'] == 0 ){
    			
    			$categories = $categories.'<a href="/products/'.$category['category_name_url'].'">'.$category['category_name'].'</a>';
						...
						if($subcategory['category_type'] == $category['category_id'])
							{
								...
								$categories = $categories.'<a href="/products/'.$subcategory['category_name_url'].'" class="sub">'.$subcategory['category_name'].'</a>';
